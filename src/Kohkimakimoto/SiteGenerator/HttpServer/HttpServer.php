<?php
/*
 * This file references Sculpin(https://sculpin.io/).
 * MIT License: (c) Dragonfly Development Inc.
 */
...
class HttpServer
{
    ...
    public function __construct(Config $config, OutputInterface $output)
    {
        ...
        $httpServer->on("request", function($request, $response) use ($self, $config, $output) {

            $path = $config->dest.'/'.ltrim(rawurldecode($request->getPath()), '/');
            if (is_dir($path)) {
                $path .= '/index.html';
            }
            if (!file_exists($path)) {
            ...
            if ('' !== $extension = pathinfo($path, PATHINFO_EXTENSION)) {
                ...
                if ($guessedType = $self->findType($extension)) {
                    $type = $guessedType;
                }
            }
        ...
        $socketServer->listen($this->config->server["port"], $this->config->server["host"]);
    ...
    public function run()
    {
        ...
        $this->output->writeln("<info>Http server started on </info><comment>http://{$host}:{$port}</comment>");
    ...
    /**
     * This code references https://github.com/dflydev/dflydev-apache-mime-types/blob/master/src/Dflydev/ApacheMimeTypes/PhpRepository.php
     * MIT License
     * @var array
     */
